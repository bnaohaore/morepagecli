...
var allcss=new ExtractTextPlugin('css/commons[hash].css');
var allless=new ExtractTextPlugin('css/commons_less[hash].css');
const yargs = require("yargs");
var ichas=yargs.argv.env.name;
var apis=yargs.argv.env.api;
var icha='../';
...
module.exports = {
  ...
  output: {  //文件输出配置
    path:path.resolve(__dirname, file_name),  //打包文件路径
    filename: 'js/[name][hash].js',  //打包文件名称build.js 多页使用[name entry为对象时的属性名][hash 每次打包的编码][chunkhash 每个文件单独的编码].js
    publicPath:icha  //引用占位符
  },
    resolve: {
  ...
  plugins: [
      ...
      new webpack.optimize.CommonsChunkPlugin({
          ...
          filename: './js/[name][hash].bundle.js', // 生成后的文件名，虽说用了[name]，但实际上就是'commons.bundle.js'了
          chunks: entries,  // chunks是需要提取的模块
          minChunks: 2 // 设定要有4个chunk（即4个页面）加载的js模块才会被纳入公共代码。这数目自己考虑，我认为3-5比较合适。
      }),
      ...
      allless,
      allcss,
      new ExtractTextPlugin('css/[name][hash].css') //提取vue里面的css
